# ============================================
# EfficientNetV2_CBAM_Transformer_v4 FINAL (OFFLINE READY)
# ============================================
...
GLOBAL_Lr = 5e-4
BATCH_SIZE = 128
MODEL_NAME = "efficientnetv2_cbam_transformer_v4"
...
# EfficientNetV2_CBAM_Transformer_v4
class EfficientNetV2_CBAM_Transformer_v4(nn.Module):
    def __init__(self, num_transformer_layers=3, num_heads=8):
        super(EfficientNetV2_CBAM_Transformer_v4, self).__init__()

        # OFFLINE → pretrained=False
        self.efficientnet = timm.create_model(
            'tf_efficientnetv2_s.in21k_ft_in1k',
            pretrained=False,   # Force offline mode
            drop_path_rate=0.2
        )

        # Optional manual weight loading (comment out if you don't have file)
        # state_dict = torch.load(r"D:\Data\NYC\Retina\model_weight\tf_efficientnetv2_s.in21k_ft_in1k.pth")
        ...
        self.pos_encoding = PositionalEncoding(d_model=self.hidden_dim, max_len=49)
...
if __name__ == "__main__":
    model = EfficientNetV2_CBAM_Transformer_v4(num_transformer_layers=3, num_heads=8)
    train_model(
        model=model,
